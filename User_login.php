...
else
{
    ...
    {
      $query = "SELECT * FROM  secondary_user WHERE user_password = '{$password}' AND  email='{$email}'";
      ...
      if(mysqli_num_rows($result)>0)
      {
          ...
          $status="ACTIVE";
          $q = "UPDATE secondary_user SET token = '{$token}' , u_status ='{$status}' where email = '{$email}'";//Updating value of token and status
