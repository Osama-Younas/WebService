...
class Logout_user extends Database
{
 function logout($key)
 {
    ...
    if(isset($key))
    {
        $query ="Select * from secondary_user where token ='{$key}'";
        ...
        if(mysqli_num_rows($result)>0)
        {
            ...
            $q = "UPDATE secondary_user SET token = '{$token}' , u_status ='{$status}' where  id='{$id}'";//Updating value of token and status
